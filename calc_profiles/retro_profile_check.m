...
ky_read = ky(read_idx); ky_read = ky_read(1:24:end);
kz_read = kz(read_idx); kz_read = kz_read(1:24:end);
dyn_read = dyn(read_idx); dyn_read = dyn_read(1:24:end);
...
figure(1); montage(kspa_frames,'displayrange',[0 3],'size',[6 9]); title('actual sampling pattern'); colormap jet;
